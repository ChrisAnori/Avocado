...
def procesar_datos_avocado(filepath, opcion='A'):
    ...
    data_avo = data_avo.drop(columns=['Unnamed: 0'])
...
def regresion_modelos(df, columnas, variable_objetivo, modelo_tipo=3,grado_polinomio=2):
    ...
    # Crear un gráfico en una subgráfica
    # Crear una figura para los 5 gráficos
    plt.figure(figsize=(15, 20))
    plt.plot(y, label='Valores Reales', marker='o', color='b')
    plt.plot(y_pred_linear, label='Predicciones', marker='x', color='r')
    ...
    results_df = pd.DataFrame({
        'Predicciones': [f"{float(pred):.2f}" for pred in y_pred_poly],
        'Valores Actuales': [f"{float(act):.2f}" for act in y],  # Agregada la coma aquí
        'Error Porcentual': [f"{float(((act - pred) / act) * 100):.2f}" for act, pred in zip(y, y_pred_poly)]
    })
    # Modelo lineal 
    results_df = pd.DataFrame({
        'Predicciones': [f"{float(pred):.2f}" for pred in y_pred_linear],
        'Valores Actuales': [f"{float(act):.2f}" for act in y],  # Agregada la coma aquí
        'Error Porcentual': [f"{float(((act - pred) / act) * 100):.2f}" for act, pred in zip(y, y_pred_linear)]
    })
    

# Imprime la tabla modelo lienal con los números formateados
    print("Modelo lineal: Predicciones, Valores Actuales , Error Porcentual :\n")
    print(results_df)
